...
const ChatBoxMessageMapper = (props) => {
  ...
  return (
    ...
    >
      ...
      {accommodations ? <Box display="flex" flexDirection="row">
        {accommodations.length > 0 ? accommodations.slice(0, 2).map((data, index) => (
          <Box key={index} onClick={() => window.open(data.link)} sx={{ cursor: 'pointer', marginRight: index !== 2 ? 2 : 0, marginLeft: index === 0 ? 0 : 2 }} minHeight={250} flex={1} mr={2} display="flex" flexDirection="column" borderRadius={4} boxShadow="0px 3.89312px 23.35869px 0px rgba(189, 189, 189, 0.23)">
            <Box flex={1}>
            ...
            <Box flex={1} minHeight={45} display="flex" flexDirection="column" alignItems="flex-start" p={2}>
              ...
              <Typography variant="caption" fontWeight={500}>
                {data.address ? data.address.substring(0, 50) : 'Description of the hotel'}
              </Typography>
            </Box>
            <Box flex={1} pl={2} pr={2} display="flex" flexDirection="row" alignItems={"flex-end"} pb={2}>
              ...
              <Box sx={{ flex: 1 }} display="flex" justifyContent="flex-end">
                <Typography variant="body1" fontWeight="bold">{data.price}€</Typography>
              </Box>
            </Box>
          </Box>
